"""带工具调用的 GRPO（ReAct 协议 + 结果奖励，批量多轮 rollout）。

rollout：模型按 ReAct 格式（Thought/Action/Action Input）多轮调用工具
（scratchpad_write/read、calculate、get_current_time），工具结果作为
<tool_response> 回填，直到不再输出 Action 或达到 max_turns。
同 batch 内所有 rollout 的同一轮次合并成一次 batched generate。
奖励：最终结果正确性为主（outcome_weight），工具算术正确比例为弱 shaping
（process_weight，默认 0.1），加格式/超轮次 shaping。
loss 只施加在 assistant 生成的 token 上（工具回填段与 prompt 被 mask 掉）。

Usage:
  cd trainer && python train_tool_grpo.py --from_weight rlvr_react \
      --data_path ../dataset/rlvr_math_v3.jsonl --save_weight rlvr_toolgrpo \
      --max_steps 2000 --use_wandb
"""
import os
import sys
...
from eval_react_math import parse_action
...
def rollout_batch(model, tokenizer, prompt_seqs, args):
    ...
    for turn in range(args.max_turns):
        ...
        for row, i in enumerate(active):
            ...
            if parsed is None or parsed[1] is None or len(completions[i]) > args.max_seq_len:
                done[i] = True
                continue
            name, call_args = parsed
            result = etm.execute_tool(name, call_args, states[i])
            n_calls[i] += 1
            if name == 'calculate':
                calc_n[i] += 1
                if 'result' in result:
...
def calculate_reward(final_text, gold, n_calls, calc_ok, calc_n, hit_cap, args):
    ...
    try:
        if pred is not None and abs(float(pred) - float(gold)) < 1e-4:
            r += args.outcome_weight
            correct = 1.0
    except (TypeError, ValueError):
        pass
    if calc_n > 0:
        r += args.process_weight * (calc_ok / calc_n)
    r += 0.2 if '####' in final_text else -0.1
    if hit_cap:
        r -= 0.1
    return r, correct


def train_epoch(model, ref_model, tokenizer, questions, golds, args, optimizer, scheduler, wandb, system_react):
    ...
    for step in range(1, iters + 1):
        ...
        if step % args.save_interval == 0:
            lm_checkpoint(args, model, None, 0, step)
...
def main():
    ...
    parser.add_argument('--save_weight', default='rlvr_toolgrpo', type=str)
    ...
    parser.add_argument("--data_path", type=str, default="../dataset/rlvr_math_v3.jsonl")
    ...
    parser.add_argument('--from_weight', default='rlvr_react', type=str)
    ...
    system_react = json.loads(open('../dataset/react_sft.jsonl').readline())['conversations'][0]['content']
    ...
    if args.use_wandb:
        ...
        wandb.init(project="MiniMind-RLVR", experiment_name=f"ToolGRPO-{args.save_weight}-B{args.batch_size}-G{args.num_generations}-LR{args.learning_rate}")

    train_epoch(model, ref_model, tokenizer, questions, golds, args, optimizer, scheduler, wandb, system_react)
