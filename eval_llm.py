...
def init_model(args):
    if args.load_from.lower() == 'none':
        ...
        print('⚠️  未加载预训练权重，模型为随机初始化，输出为乱码属正常现象')
    elif 'model' in args.load_from:
...
def main():
    ...
    parser.add_argument('--load_from', default='model', type=str, help="模型加载路径（none=随机初始化不加载权重，model=原生torch权重，其他路径=transformers格式）")
