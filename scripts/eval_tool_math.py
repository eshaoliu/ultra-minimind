...
def rollout(model, tokenizer, question, instr, args):
...
def main():
    ...
    correct = used_tool = fmt_ok_n = has_ans_mark = 0
    total_calls = 0
    results = []
    for i, s in enumerate(samples):
        final_text, n_calls, fmt_ok, trace = rollout(model, tokenizer, s['q'], args.instr, args)
        pred = extract_answer(final_text)
        pred_v, gold_v = to_float(pred), to_float(s['a'])
        ok = pred_v is not None and gold_v is not None and abs(pred_v - gold_v) < 1e-4 * max(1.0, abs(gold_v))
        correct += ok
        used_tool += n_calls > 0
        fmt_ok_n += fmt_ok
        has_ans_mark += '####' in final_text
        total_calls += n_calls
        results.append({'q': s['q'][:200], 'gold': s['a'], 'gold_num': gold_v,
                        'pred': pred, 'pred_num': pred_v,
                        'correct': ok, 'n_calls': n_calls, 'trace': trace,
                        'final': final_text[-300:]})
        print(f'[{i+1}/{len(samples)}] gold={s["a"]}({gold_v}) pred={pred}({pred_v}) '
    ...
    print(f'带 #### 终答: {has_ans_mark}/{n}')
    wstem = os.path.basename(args.weight).split('.')[0]
    dstem = os.path.basename(args.data).split('.')[0]
    dump = args.dump or f'/root/autodl-tmp/minimind/out/tool_math_eval_{wstem}_{dstem}.json'
    with open(dump, 'w') as f:
        json.dump({'acc': correct / n, 'n': n, 'used_tool': used_tool,
                   'avg_calls': total_calls / n, 'fmt_ok': fmt_ok_n,
                   'has_ans_mark': has_ans_mark, 'instr': args.instr[:40],
